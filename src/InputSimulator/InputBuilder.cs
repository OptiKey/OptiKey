    ...
    internal class InputBuilder : IEnumerable<INPUT>
    {
        ...
        public static bool IsExtendedKey(VirtualKeyCode keyCode)
        {
            if (keyCode == VirtualKeyCode.MENU ||
                //keyCode == VirtualKeyCode.LMENU || //JS: OSK does not add the extended flag to LEFT ALT - removed
                //keyCode == VirtualKeyCode.RMENU || //JS: OSK does not add the extended flag to RIGHT ALT - removed
                keyCode == VirtualKeyCode.LWIN || //JS: OSK adds the extended flag to LEFT WIN - added
                keyCode == VirtualKeyCode.CONTROL || //JS: OSK adds the extended flag to RIGHT CONTROL, but not LEFT CONTROL - cannot test CONTROL so leaving for now
        ...
        /// <summary>
        /// Gets the key code to be included in the keyboard input for the supplied <see cref="VirtualKeyCode"/>
        /// </summary>
        /// <param name="keyCode">The key code.</param>
        /// <returns>UInt16 != 0 if a key code should be included</returns>
        public static UInt16 GetKeyCodeForInclusion(VirtualKeyCode keyCode)
        {
            if (keyCode == VirtualKeyCode.DELETE ||
                keyCode == VirtualKeyCode.LEFT ||
                keyCode == VirtualKeyCode.RIGHT ||
                keyCode == VirtualKeyCode.UP ||
                keyCode == VirtualKeyCode.DOWN ||
                                    ...
                                    new KEYBDINPUT
                                        {
                                            KeyCode = keyCodeParam,
                                            Scan = scanParam,
                                            Flags = flagParam | (UInt32)KeyboardFlag.KeyUp,
                                            Time = 0,
                                            ExtraInfo = IntPtr.Zero
                                        }
                            }
