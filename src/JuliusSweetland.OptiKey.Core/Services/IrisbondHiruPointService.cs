...
{
    public class IrisbondHiruPointService : IPointService
    {
        #region Fields
        ...
        public event EventHandler<Timestamped<Point>> Point
        {
            add
            {
                if (pointEvent == null)
                {
                    Log.Info("Checking the Irisbond Hiru tracker is connected...");
            ...
            if (pointEvent != null)
            {
                   ...
                   && !double.IsNaN(y))
                {
                    pointEvent(this, new Timestamped<Point>(new Point(pogX, pogY),
                        new DateTimeOffset(DateTime.UtcNow).ToUniversalTime())); //IrisbondHiru does not publish a useable timestamp
                }
            }
        }
