    ...
    public partial class DynamicKeyboard : KeyboardView
    {
        ...
        private bool SetupDynamicItems()
        {
            ...
            findAvailableRow:
                ...
                findAvailableColumn:
                    ...
                    {
                        while (openGrid.ElementAt(row).First() < dynamicItem.Col)
                        {
                            openGrid.ElementAt(row).RemoveAt(0);
                        }
                    }
        ...
        private void SetupDynamicItem(XmlDynamicItem dynamicItem, int minKeyWidth, int minKeyHeight)
        {
            ...
            else if (dynamicItem is XmlDynamicScratchpad xmlScratchpadItem)
            {
                ...
                {
                    scratchpad.Background = (SolidColorBrush)new BrushConverter().ConvertFrom(dynamicItem.BackgroundColor);
                }

                double vOpacity = 1;
                if (!string.IsNullOrEmpty(dynamicItem.Opacity) && double.TryParse(dynamicItem.Opacity, out vOpacity))
                {
                    scratchpad.Opacity = vOpacity;
                }
            }
            else if (dynamicItem is XmlDynamicSuggestionRow xmlSuggestionRow)
            {
                ...
                if (!string.IsNullOrEmpty(dynamicItem.BackgroundColor)
                    ...
                    || System.Drawing.Color.FromName(dynamicItem.BackgroundColor).IsKnownColor))
                {
                    suggestionRow.Background = (SolidColorBrush)new BrushConverter().ConvertFrom(dynamicItem.BackgroundColor);
                }

                double vOpacity = 1;
                if (!string.IsNullOrEmpty(dynamicItem.Opacity) && double.TryParse(dynamicItem.Opacity, out vOpacity))
                {
                    suggestionRow.Opacity = vOpacity;
                }
            }
        ...
        private string AddDynamicStepList(XmlDynamicKey xmlDynamicKey, int minKeyWidth, int minKeyHeight)
        {
            ...
            if (xmlDynamicKey.Steps.Count > 0)
            {
                ...
                foreach (XmlDynamicKey vStep in xmlDynamicKey.Steps)
                {
                    ...
                    //else if (vStep is XmlDynamicKey vkeyLoop) //this is last because all types are XmlDynamicKey, but we only want to go deeper if we hit a Loop flag
                    //{
