...
namespace JuliusSweetland.OptiKey.UI.ViewModels
{
    public partial class MainViewModel
    {
        ...
        private async Task StepListCommands(KeyValue singleKeyValue, List<string> multiKeySelection, List<string> commandList)
        {
            ...
            while (commandList.Any())
            {
                ...
                else
                {
                    ...
                    else if (commandList.First().StartsWith("Keyboard>") || commandList.First().StartsWith("KeyboardAndReturn>"))
                    {
                        ...
                        int vIndex = (commandList.First().StartsWith("KeyboardAndReturn>")) ? 18 : 9;
                        ...
                        {
                            var keyboardString = commandList.First().Substring(vIndex);
                            Log.InfoFormat("StepListCommand changing keyboard to string {0}. Replace previous keyboard={1}", keyboardString, vReturn);
                            commandKeyValue = new ChangeKeyboardKeyValue(keyboardString, vReturn);
                        }
                        KeySelectionResult(commandKeyValue, multiKeySelection);
                    }
                    else if (commandList.First().StartsWith("KeyDown>"))
                    {
                        var commandKeyString = commandList.First().Substring(8);
                        Log.InfoFormat("StepListCommand: key down on {0} key", commandKeyString);
                        commandKeyValue = new KeyValue(commandKeyString);

                        //keyboardOutputService.ProcessSingleKeyPress is not implemented so commenting out for now:
                        //int delayMs = 0;
                        //await keyboardOutputService.ProcessSingleKeyPress(keyValue.String, KeyPressKeyValue.KeyPressType.Press, delayMs);

                        if (keyStateService.KeyDownStates[commandKeyValue].Value != KeyDownStates.Down)
                        {
                    ...
                    {
                        var commandKeyString = commandList.First().Substring(6);
                        Log.InfoFormat("StepListCommand: key up on {0} key", commandKeyString);
                        commandKeyValue = new KeyValue(commandKeyString);

                        //keyboardOutputService.ProcessSingleKeyPress is not implemented so commenting out for now:
                        //int delayMs = 0;
                        //await keyboardOutputService.ProcessSingleKeyPress(keyValue.String, KeyPressKeyValue.KeyPressType.Release, delayMs);
                    ...
                    {
                        var commandKeyString = commandList.First().Substring(10);
                        Log.InfoFormat("StepListCommand: key toggle on {0} key", commandKeyString);
                        commandKeyValue = new KeyValue(commandKeyString);

                        if (keyStateService.KeyDownStates[commandKeyValue].Value != KeyDownStates.Up)
                        {
                            Log.Info("StepListCommand: ...toggling key up");
                            //keyboardOutputService.ProcessSingleKeyPress is not implemented so commenting out for now:
                            //int delayMs = 0;
                            //await keyboardOutputService.ProcessSingleKeyPress(keyValue.String, KeyPressKeyValue.KeyPressType.Release, delayMs);
                            keyStateService.KeyDownStates[commandKeyValue].Value = KeyDownStates.Up;
                        ...
                        {
                            Log.Info("StepListCommand: ...toggling key down");
                            //keyboardOutputService.ProcessSingleKeyPress is not implemented so commenting out for now:
                            //int delayMs = 0;
                            //await keyboardOutputService.ProcessSingleKeyPress(keyValue.String, KeyPressKeyValue.KeyPressType.Press, delayMs);
                            keyStateService.KeyDownStates[commandKeyValue].Value = KeyDownStates.Down;
                    ...
                    {
                        var text = commandList.First().Substring(5);
                        Log.InfoFormat("StepListCommand: text of '{0}'", text);
                        commandKeyValue = new KeyValue(text);
                        KeySelectionResult(commandKeyValue, multiKeySelection);
                    }
